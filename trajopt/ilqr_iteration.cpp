...
int main(){
    ...
    //initialize the mag field/ moment
    Vector3d B(1,1,1);
    Vector3d m(0,1,0);

    //initialize the next step
    VectorXd y_new(7);
    y_new = rk4(t0, h, y0, I, B, m);

    cout << "y_new = " << endl << y_new << endl;
...
VectorXd state_dot(VectorXd y, MatrixXd I, Vector3d B, Vector3d m){
    ...
    c = I*omega;
    y_dot.segment<3>(0) = I.inverse() * (-omega.cross(c) + m.cross(B)); // generate omega_dot
