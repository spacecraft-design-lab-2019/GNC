...
def iLQRsimple_py(x0, xg, utraj0, Q, R, Qf, dt, tol):
	...
	for k in range(0, N-1):
		J = J + 0.5 * (xtraj[:, k] - xg) @ Q @ (xtraj[:, k] - xg) + 0.5 * R*utraj[:, k] @ utraj[:, k]
		xtraj[:, k+1], A[:, Nx*k:Nx*(k+1)], B[:, Nx*k:Nx*(k+1)] = rkstep_py(xtraj[:, k], utraj[:, k], dt)
	...
	while np.amax(np.abs(l)) > tol:
		...
		for k in range(N-2, -1, -1):
			...
			snew = q - Kk.T @ r + R * Kk.T @ l[:, k] + (Ak - Bk @ Kk).T @ (s - S @ Bk @ l[:, k])
			S = Snew
			s = snew
		...
		while (Jnew > J):
			Jnew = 0
			for k in range(0, N-1):
				unew[:, k] = utraj[:, k] - alpha*l[:, k] - K[:, Nx*k:Nx*(k+1)] @ (xnew[:, k] - xtraj[:, k])
				xnew[:, k+1], A[:, Nx*k:Nx*(k+1)], B[:, Nx*k:Nx*(k+1)] = rkstep_py(xnew[:, k], unew[:, k], dt)
